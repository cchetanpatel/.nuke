...
class ParseNodes():
    ...
    def getGeoNodes( self ):
        ...
        geo_nodes = [ n for n in self.nodes if (set( ('display', 'render_mode') ).issubset( set( n.knobs() ) ) or n.Class()=='PoissonMesh') and n.Class()!='Scene']
    ...
    def getProjNodes( self ):
        ...
        for node in self.nodes:
            if node.Class() in ('Project3D', 'Project3D2'):
...
def getNumericVersion(version_str):
    ...
    The integers generated in this way will match mari.app.version().number() in scale.
    """
...
def createMariTab():
    ...
    end_group_knob = createKnob(node, nuke.Tab_Knob, 'endGroup', 'endGroup', -1)    # -1 = "make it an end group knob"
